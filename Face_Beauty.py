...
def TencentFaceBeauty(pic_str, white, smooth, facelift, eyeenlarge):
    try:
        cred = credential.Credential("AKIDPahlcEegusa5KwJSTs43nmhRvL4YZnXM", "qRegT0G59zaw46sN7WgaCdzsc7QehosM")
...
def About():
    ...
    os.remove('temp.gif')
    ttk.Label(software_frame, text="Face Beauty Version 1.1").grid(row=0, column=1, sticky = W)
    ttk.Label(software_frame, text="@Author    :   lijishi").grid(row=1, column=1, sticky = W)
    ttk.Label(software_frame, text="@EditTime  :   Jan 25,2020").grid(row=2, column=1, sticky = W)
...
Whitening = tk.Scale(option_frame, label='美白程度', from_=0, to=100,
                     orient=tk.HORIZONTAL, length=180, tickinterval=20, resolution=1)
Whitening.grid(row = 2, column = 0, columnspan = 2, padx=10, pady=5, sticky = W)
Smoothing = tk.Scale(option_frame, label='磨皮程度', from_=0, to=100,
                     orient=tk.HORIZONTAL, length=180, tickinterval=20, resolution=1)
Smoothing.grid(row = 2, column = 3, columnspan = 2, padx=14, pady=5, sticky = W)
FaceLifting = tk.Scale(option_frame, label='瘦脸程度', from_=0, to=100,
                     orient=tk.HORIZONTAL, length=180, tickinterval=20, resolution=1)
FaceLifting.grid(row = 3, column = 0, columnspan = 2, padx=10, pady=5, sticky = W)
EyeEnlarging = tk.Scale(option_frame, label='大眼程度', from_=0, to=100,
                     orient=tk.HORIZONTAL, length=180, tickinterval=20, resolution=1)
EyeEnlarging.grid(row = 3, column = 3, columnspan = 2, padx=14, pady=5, sticky = W)
Whitening.set(30)
Smoothing.set(10)
FaceLifting.set(70)
EyeEnlarging.set(70)
ttk.Button(main_window, text = "开始", width = 25, command = Beauty).grid(row = 6, column = 0, columnspan = 2, padx = 10)
ttk.Button(main_window, text = "提示", command = Tip).grid(row = 6, column = 2)
ttk.Button(main_window, text = "关于", command = About).grid(row = 6, column = 3)
...
ttk.Button(main_window,text = "提示", command = Tip).grid(row = 3, column = 5)
